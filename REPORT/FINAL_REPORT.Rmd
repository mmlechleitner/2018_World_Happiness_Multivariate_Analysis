...
Quick test for heteroskedasticity:
```{r, echo = TRUE, fig.width=8, fig.height=8}
require(lmtest)
#results when not doing any transformation:
...
sub <- data.frame(Happiness_Score = full_best_df$Happiness_Score, dim1= res.pca$ind$coord[,1])
sub$Happiness_Score <- sub$Happiness_Score + 3

distBCMod <- caret::BoxCoxTrans(sub$Happiness_Score)
print(distBCMod)
sub <- cbind(sub, happy_new=predict(distBCMod, sub$Happiness_Score)) # append the transformed variable to cars
lmMod_bc <- lm(happy_new ~ dim1, data=sub)
bptest(lmMod_bc)
plot(lmMod_bc)
```
