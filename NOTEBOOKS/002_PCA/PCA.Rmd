...
#temp_df <- full_best_df %>%  select(-X,-Country,-Region,-Happiness_Score,-Total_Population,-Mental_and_Substance_Disorder_Index,-Compulsory_Education_in_Years, -Agricultural_Land_Percentage,-Generosity,-Forest_Area_Land_Percentage,-Suicide_Index,-Legal_Rights_Index,-Infant_Immunization_Measles_Index)

res.pca <- PCA(temp_df,graph=F) #Biplot of PC1 and PC2
print(res.pca)
```
...
```{r, echo = TRUE, fig.width=8, fig.height=8}
# Contribution of variables to each Principal Components:
dimdesc(res.pca) 

#Scree plot of the explained variance of the first 5 Principal Components:
...
corrplot(res.pca$eig, is.corr=F, method="number")
...
ggcorrplot(var$cos2, lab=T, insig ="blank") + coord_flip() + ggtitle("Quality of Representation per Dimension")

corrplot(var$cos2, is.corr=F, method="number", title="Qualirt of representation per Dimension", mar=c(0,0,1,0))
...
fit <-rpart(Happiness_Score~ ., data=train.sample, method= "anova")
