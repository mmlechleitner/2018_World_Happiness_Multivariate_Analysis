...
temp_df <- full_best_df %>% 
  select(-X,-Country,-Region,-Happiness_Score,
         -Total_Population,-Mental_and_Substance_Disorder_Index,-Compulsory_Education_in_Years,-Agricultural_Land_Percentage,-Generosity,-Forest_Area_Land_Percentage,-Suicide_Index,-Legal_Rights_Index,-Air_Pollution)
```
...
#  Grouping countries by their "happiness" on the biplot 
fviz_pca_ind(res.pca, col.ind = target_Factorised$Happiness_Score, 
...
As discovered, the 1st and 3rd pcs give much better results than the 1st combined with the 2nd pcs!:
```{r, echo = TRUE, fig.width=8, fig.height=8}
fitfirstthirdpcs <- lm(happiness_score~ Dim.1 + Dim.3, data=predictors)
summary(fitfirstthirdpcs)
