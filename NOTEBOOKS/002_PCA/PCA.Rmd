...
To reduce the dimensionality, we are performing hereafter the PCA multivariate technique. In short, it consists of transforming the current variables into a set of principal components, which are uncorrelated between each other. Each Principal Component correspond to a linear combination of the original variables. 
...
First, we are calling the PCA function, which shows two correlation biplots of PC1 & PC2. The first one is the graphical representation of the set of variables while the second one is of the countries. 
```{r, echo = TRUE, fig.width=8, fig.height=8}
res.pca <- PCA(temp_df,graph=T) #Biplot of PC1 and PC2
print(res.pca)
```

Scree Plot showing the percentage of explained variance per principal component and secondly the contribution of each variable to each component. 
```{r, echo = TRUE, fig.width=8, fig.height=8}
